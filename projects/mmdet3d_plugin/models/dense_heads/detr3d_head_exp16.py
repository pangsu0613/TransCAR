...
class Detr3DHead(DETRHead):
    ...
    def forward(self, mlvl_feats, img_metas):
        ...
        ffn_out = self.rf_linear2(self.rf_dropout(self.rf_activation(self.rf_linear1(query_feat_input))))
        query_feat_input = query_feat_input + self.rf_dropout3(ffn_out)
        query_feat_input = self.rf_norm3(query_feat_input)

        #query_feat[nan_row_index] = query_feat_input

        query_feat_input = query_feat_input.permute(1,0,2)




        #output_final_class = self.final_cls(query_feat)
        tmp = self.final_reg(query_feat_input)
        ...
        ## tmp shape [1,470,10], reference shape [1,900,3]
        ...
        tmp[..., 0:2] += reference[0,nan_row_index, 0:2]
        #tmp[..., 0:2] = tmp[..., 0:2].sigmoid()
        tmp[..., 4:5] += reference[0,nan_row_index, 2:3]
        #tmp[..., 4:5] = tmp[..., 4:5].sigmoid()
        #tmp[..., 0:1] = (tmp[..., 0:1] * (self.pc_range[3] - self.pc_range[0]) + self.pc_range[0])
        #tmp[..., 1:2] = (tmp[..., 1:2] * (self.pc_range[4] - self.pc_range[1]) + self.pc_range[1])
        #tmp[..., 4:5] = (tmp[..., 4:5] * (self.pc_range[5] - self.pc_range[2]) + self.pc_range[2])
        ...
        outputs_classes = [outputs_classes[-1]]
        outputs_coords = [outputs_coords[-1]]
        #print(outputs_coords[0].shape, tmp.shape)
        #outputs_coords[0]  [1,900,10]
        outputs_coords[0][0,nan_row_index,:] = tmp
