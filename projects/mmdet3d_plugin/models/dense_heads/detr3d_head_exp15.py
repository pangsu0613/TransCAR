...
class Detr3DHead(DETRHead):
    ...
    def forward(self, mlvl_feats, img_metas):
        ...
        #print(radar_points.shape) # [1, 1500, 3]
        radar_points[0,:,0] = (radar_points[0,:,0] - self.pc_range[0]) / (self.pc_range[3] - self.pc_range[0])
        radar_points[0,:,1] = (radar_points[0,:,1] - self.pc_range[1]) / (self.pc_range[4] - self.pc_range[1])
        radar_points[0,:,2] = (radar_points[0,:,2] - self.pc_range[2]) / (self.pc_range[5] - self.pc_range[2])
        radar_pos_feat = self.radar_position_encoder(inverse_sigmoid(radar_points))
