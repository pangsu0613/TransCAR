...
class Detr3DHead(DETRHead):
    ...
    def forward(self, mlvl_feats, img_metas):
        ...
        #print("tmp perspective:",tmp[0,100,[0,1,4]])
        ...
        reference = inter_references[-1]  #[1,900,3]
        #print("length:",len(inter_references))
        #print("reference:",torch.min(reference),torch.max(reference))
        reference_points_clone = reference.clone()  #！！这里有问题！！ 和上面不一样，inter_references本来就在sigmoid domain.
        ...
        #print("inter_references perspective:",reference_points_clone[0,100,:])
        ...
        '''
        print("111111111111111111111",torch.isnan(query_feat).any(),torch.isnan(reference).any())
        print(dist[0,100,200],"radar",radar_tokens[0,200,:2],"query",reference_points_clone[0,100,:2])
        print("radar and reference",torch.isnan(radar_tokens).any(),torch.isnan(reference_points_clone).any())
        print("reference",torch.isnan(reference).any())'''
        attention_mask = dist[0]>4.0
        #print(attention_mask.shape)

        nan_row_index = torch.where((attention_mask == False).any(dim=1))[0]
        new_attention_mask = attention_mask[nan_row_index]
        #print(nan_row_index.shape)
        #print(new_attention_mask.shape)
        #print((new_attention_mask==True).all(dim=1)==True)
        query_feat_input = query_feat[nan_row_index]

        radar_pos_feat = radar_pos_feat.permute(1,0,2)
        tgt2,att_score = self.rf_multihead_attn(query_feat_input, radar_pos_feat, radar_pos_feat,
                                                #key_padding_mask=k_padding_mask, attn_mask = new_attention_mask)#[0]
                                                attn_mask = new_attention_mask)

        #print("!!!!!!!!!!!!!!",torch.where(torch.isnan(att_score[0])))  #att_score [1,900,1500]
        #ind = torch.where(torch.isnan(att_score[0]))
        #print(new_attention_mask[ind])
        #tgt2 = torch.nan_to_num(tgt2)
        #att_score = torch.nan_to_num(att_score)
        #radar_pos_feat = torch.nan_to_num(radar_pos_feat)
        #print("222222222222222222222",torch.isnan(tgt2).any(),torch.isnan(att_score).any(),torch.isnan(query_feat).any(),torch.isnan(reference).any(),torch.isnan(radar_pos_feat).any())
        query_feat_input = query_feat_input + self.rf_dropout2(tgt2)
        query_feat_input = self.rf_norm2(query_feat_input)
        tgt2 = self.rf_linear2(self.rf_dropout(self.rf_activation(self.rf_linear1(query_feat_input))))
        query_feat_input = query_feat_input + self.rf_dropout3(tgt2)
        query_feat_input = self.rf_norm3(query_feat_input)
        ...
        filename = '/home/pangsu/Downloads/detr3d/att_plot/raw_output_exp13/' + sample_idx + '.pkl'
