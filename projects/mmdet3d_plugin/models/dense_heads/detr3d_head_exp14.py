...
class Detr3DHead(DETRHead):
    ...
    def __init__(self,
        ...
        self.query_position_encoder = nn.Sequential(
            nn.Linear(3, self.embed_dims),
            nn.LayerNorm(self.embed_dims),
            nn.ReLU(inplace=True),
            nn.Linear(self.embed_dims, self.embed_dims),
    ...
    def forward(self, mlvl_feats, img_metas):
        ...
        q_residual = hs[5].permute(1,0,2).clone()
        #print("first::",query_feat[100,0,:5])
        ...
        query_pos = self.query_position_encoder(reference_points_clone).permute(1, 0, 2) # if unfreeze the transformer, shoud use reference
        #print(query_pos.shape,query_feat.shape,q_residual.shape)
        #print("after::",q_residual[100,0,:5])
        final_query_feat = self.q_dropout(query_feat) + q_residual + query_pos
        final_query_feat = final_query_feat.permute(1,0,2)




        output_final_class = self.final_cls(final_query_feat)
        tmp = self.final_reg(final_query_feat)
