...
nusc = NuScenes(version='v1.0-mini', dataroot='/home/xi/Downloads/detr3d/data/nuscenes', verbose=True)
#nusc = NuScenes(version='v1.0-trainval', dataroot='/home/xi/Downloads/detr3d/data/nuscenes', verbose=True)
...
class Detr3DHead(DETRHead):
    ...
    def forward(self, mlvl_feats, img_metas):
        ...
        #radar_pointcloud_frontr, timestamps_list_frr = RadarPointCloud.from_file_multisweep(self.nusc, sample_instance, chan="RADAR_FRONT", ref_chan="RADAR_FRONT", nsweeps=5)
        #print("lidar:",radar_pointcloud_front.points[:,3])
        #print("radar",radar_pointcloud_frontr.points[:,3])
        ####
        ...
        #print("time:",timestamps_list_f.shape)  #[1,N]
        #print("points:",radar_pointcloud_front.points.shape) #[18,N]
        #######################################################  Radar front
        ...
        ########################################################
        #print(ambig_state_f_1h[:10,:])
        #print(dyn_prop_f_1h[:10,:])
        # (0)x (1)y (2)z (3)dyn_prop (4)id (5)rcs (6)vx (7)vy (8)vx_comp (9)vy_comp (10)is_quality_valid (11)ambig_state (12)x_rms (13)y_rms (14)invalid_state (15)pdh0 (16)vx_rms (17)vy_rms

        radar_front_xyz = radar_pointcloud_front.points.transpose()[:,[0,1,2,4,5,6,7,8,9,10,12,13,14,16,17]] # 15 digits # [0,1,2,8,9] x y z vx_comp vy_comp
        radar_front_left_xyz = radar_pointcloud_front_left.points.transpose()[:,[0,1,2,4,5,6,7,8,9,10,12,13,14,16,17]]  # [0,1,2,8,9]
        radar_front_right_xyz = radar_pointcloud_front_right.points.transpose()[:,[0,1,2,4,5,6,7,8,9,10,12,13,14,16,17]]  # [0,1,2,8,9]
        radar_back_left_xyz = radar_pointcloud_back_left.points.transpose()[:,[0,1,2,4,5,6,7,8,9,10,12,13,14,16,17]]  # [0,1,2,8,9]
        radar_back_right_xyz = radar_pointcloud_back_right.points.transpose()[:,[0,1,2,4,5,6,7,8,9,10,12,13,14,16,17]]  # [0,1,2,8,9]
        radar_feat_num = radar_front_xyz.shape[1]

        radar_front_xyz = np.concatenate((radar_front_xyz,dyn_prop_f_1h,ambig_state_f_1h,pdh0_f_1h),axis=1)
        ...
        save_dict['referece_points'] = new_reference_3d
        save_dict['nan_row_index'] = nan_row_index2
        filename = '/home/xi/Downloads/detr3d/att_plot/raw_output_exp20/' + sample_idx + '.pkl'
