...
def prepare_mask(tmp, eps_range, eps_rot):
    '''
    4-----1     y
    |     |     |-->x
    3-----2
    ...
    tmpc_corners_1x = tmpc[...,0] + (tmpc[...,3].exp()/2 * tmpc[...,7]) - (tmpc[...,2].exp()/2 * tmpc[...,6]) # x+l/2*rot_consine - w/2*rot_sine
    tmpc_corners_2x = tmpc[...,0] + (tmpc[...,3].exp()/2 * tmpc[...,7]) + (tmpc[...,2].exp()/2 * tmpc[...,6]) # x+l/2*rot_consine + w/2*rot_sine
    tmpc_corners_3x = tmpc[...,0] - (tmpc[...,3].exp()/2 * tmpc[...,7]) + (tmpc[...,2].exp()/2 * tmpc[...,6]) # x-l/2*rot_consine + w/2*rot_sine
    tmpc_corners_4x = tmpc[...,0] - (tmpc[...,3].exp()/2 * tmpc[...,7]) - (tmpc[...,2].exp()/2 * tmpc[...,6]) # x-l/2*rot_consine - w/2*rot_sine
    # [1,900]
    tmpc_corners_1y = tmpc[...,1] + (tmpc[...,3].exp()/2 * tmpc[...,6]) + (tmpc[...,2].exp()/2 * tmpc[...,7]) # x+l/2*rot_consine - w/2*rot_sine
    tmpc_corners_2y = tmpc[...,1] + (tmpc[...,3].exp()/2 * tmpc[...,6]) - (tmpc[...,2].exp()/2 * tmpc[...,7]) # x+l/2*rot_consine + w/2*rot_sine
    tmpc_corners_3y = tmpc[...,1] - (tmpc[...,3].exp()/2 * tmpc[...,6]) - (tmpc[...,2].exp()/2 * tmpc[...,7]) # x-l/2*rot_consine + w/2*rot_sine
    tmpc_corners_4y = tmpc[...,1] - (tmpc[...,3].exp()/2 * tmpc[...,6]) + (tmpc[...,2].exp()/2 * tmpc[...,7]) # x-l/2*rot_consine - w/2*rot_sine
    ...
    #print(tmpc_corners1[0,100,:])
    #print(tmpc_corners2[0,100,:])
    #print(tmpc_corners3[0,100,:])
    #print(tmpc_corners4[0,100,:])
    #corners' ranges
    ...
    return tmpc_range_cat, tmpc_rot_cat, tmpc_range_mid_cat, tmpc_rot_mid_cat,  tmpc_range_diff, tmpc_rot_diff
...
class Detr3DHead(DETRHead):
    ...
    def forward(self, mlvl_feats, img_metas):
        ...
        tmpc_range_cat, tmpc_rot_cat, tmpc_range_mid_cat, tmpc_rot_mid_cat, tmpc_range_diff, tmpc_rot_diff = prepare_mask(tmp, 0.25, 0.0)
        # calculate Distance
        ...
        '''
        # for debug
        nan_row_index = torch.where((final_mask == True).any(dim=1))[0]
        print("reference:",reference_points_clone[0,nan_row_index[2],:])
        ri = final_mask[nan_row_index[2]]
        print(ri.shape)
        print("radar:",radar_tokens[0,ri,:])'''
        ...
        #+++++++++++++++++++++++++++ frustum idea, Pang  06/20/2022
        tmpc_range_cat2, tmpc_rot_cat2, tmpc_range_mid_cat2, tmpc_rot_mid_cat2, tmpc_range_diff2, tmpc_rot_diff2 = prepare_mask(tmp, 0.125, 0.0)
        # calculate Distance
        ...
        '''
        # for debug
        nan_row_index2 = torch.where((final_mask2 == True).any(dim=1))[0]
        print("reference:",new_reference_3d[0,nan_row_index2[2],:])
        ri2 = final_mask2[nan_row_index2[2]]
        print(ri2.shape)
        print("radar:",radar_tokens[0,ri2,:])'''
        attention_mask2 = ~final_mask2
