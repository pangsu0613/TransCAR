...
class Detr3DHead(DETRHead):
    ...
    def forward(self, mlvl_feats, img_metas):
        ...
        radar_front_xyz = np.concatenate((radar_front_xyz, velocities_f.transpose()[:,:2], velocities_f2.transpose()[:,:2],dyn_prop_f_1h, ambig_state_f_1h, pdh0_f_1h),axis=1)
        radar_front_left_xyz = np.concatenate((radar_front_left_xyz, velocities_fl.transpose()[:,:2], velocities_fl2.transpose()[:,:2],dyn_prop_fl_1h, ambig_state_fl_1h, pdh0_fl_1h),axis=1)
        radar_front_right_xyz = np.concatenate((radar_front_right_xyz, velocities_fr.transpose()[:,:2], velocities_fr2.transpose()[:,:2],dyn_prop_fr_1h, ambig_state_fr_1h, pdh0_fr_1h),axis=1)
        radar_back_left_xyz = np.concatenate((radar_back_left_xyz, velocities_bl.transpose()[:,:2], velocities_bl2.transpose()[:,:2],dyn_prop_bl_1h, ambig_state_bl_1h, pdh0_bl_1h),axis=1)
        radar_back_right_xyz = np.concatenate((radar_back_right_xyz, velocities_br.transpose()[:,:2], velocities_br2.transpose()[:,:2],dyn_prop_br_1h, ambig_state_br_1h, pdh0_br_1h),axis=1)
        radar_feat_num = radar_front_xyz.shape[1]
        ...
        #print("initla:",query_feat_input.shape)
        combined_radar_feat = combined_radar_feat.permute(1,0,2)

        #---------------------------------------------for testing no transformer
        #print("new_attention_mask",new_attention_mask.shape) # [M, 1500]
        #print("combined_radar_feat",combined_radar_feat.shape)     # [1500, 1, 256]
        new_attention_mask_mat = (~new_attention_mask).reshape(1,-1,1500).type_as(combined_radar_feat)
        combined_radar_feat = combined_radar_feat.permute(1,0,2)
        sum_radar_feat = torch.matmul(new_attention_mask_mat,combined_radar_feat)
        radar_num_per_query = torch.sum(~new_attention_mask,dim=1).reshape(-1,1)
        radar_num_per_query = radar_num_per_query.repeat(1,256).reshape(1,-1,256)
        query_feat_input = query_feat_input + self.rf_dropout2((sum_radar_feat/radar_num_per_query).permute(1,0,2))
        ...
        query_feat[nan_row_index] = query_feat_input#query_feat[nan_row_index] + self.rf_dropout2(tgt2)
        ...
        #print("zero::",query_feat[nan_row_index2])
        #---------------------------------------------for testing no transformer
        #print("new_attention_mask",new_attention_mask.shape) # [M, 1500]
        #print("combined_radar_feat",combined_radar_feat.shape)     # [1500, 1, 256]
        new_attention_mask_mat2 = (~new_attention_mask2).reshape(1,-1,1500).type_as(combined_radar_feat)
        #combined_radar_feat = combined_radar_feat.permute(1,0,2)
        sum_radar_feat2 = torch.matmul(new_attention_mask_mat2,combined_radar_feat)
        radar_num_per_query2 = torch.sum(~new_attention_mask2,dim=1).reshape(-1,1)
        radar_num_per_query2 = radar_num_per_query2.repeat(1,256).reshape(1,-1,256)
        query_feat_input2 = query_feat_input2 + self.rf_dropout2_2((sum_radar_feat2/radar_num_per_query2).permute(1,0,2))
        #print("second::",query_feat_input2)
        #print("third::",query_feat[nan_row_index2])
        #print("firs5t:",query_feat_input2 == query_feat[nan_row_index2])
        #print("second:",query_feat[nan_row_index2])
        #combined_radar_feat = combined_radar_feat.permute(1,0,2)
        ...
        query_feat[nan_row_index2] = query_feat_input2
        #query_feat_input = query_feat_input + self.rf_dropout2(tgt2)
