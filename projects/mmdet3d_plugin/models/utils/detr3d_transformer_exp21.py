...
class Detr3DTransformer(BaseModule):
    ...
    def forward(self,
        ...
        reference_points[..., 0:1] = (reference_points[..., 0:1] * (point_range[3] - point_range[0]) + point_range[0])
        reference_points[..., 1:2] = (reference_points[..., 1:2] * (point_range[4] - point_range[1]) + point_range[1])
        reference_points[..., 4:5] = (reference_points[..., 4:5] * (point_range[5] - point_range[2]) + point_range[2])
        #print(reference_points.requires_grad,reference_points.device)
        #====================================================================================================================

        init_reference_out = reference_points   # now in 3D space
        ...
        for lid, layer in enumerate(self.layers):
            ...
            output = layer(
                ...
                *args,
                ...
                radar_features=combined_radar_feat,
            ...
            if reg_branches is not None:
                ...
                new_reference_points[..., 2:3] = tmp[
                    ..., 4:5] + (reference_points[..., 2:3])

                #new_reference_points = new_reference_points.sigmoid()

                reference_points = new_reference_points.detach()  # in 3D space
...
class Detr3DCrossAtten(BaseModule):
    ...
    def forward(self,
        ...
        #===========================================================================================
        reference_points_clone = reference_points_3d.clone()
        ...
        #print(torch.min(reference_points_clone),torch.max(reference_points_clone),torch.min(radar_tokens),torch.max(radar_tokens))
        attention_mask = dist[0]>1.0

        nan_row_index = torch.where((attention_mask == False).any(dim=1))[0]
        new_attention_mask = attention_mask[nan_row_index]

        query_feat = self.dropout(output) + inp_residual + pos_feat

        query_feat_input = query_feat[nan_row_index]

        radar_features = radar_features.permute(1,0,2)
        tgt2,att_score = self.rf_multihead_attn(query_feat_input, radar_features, radar_features,
